  ...
  pomelo.init({host: 'localhost', port: '3010'}, function () {
    ...
    request('connector.entryHandler.entry', {uid: uid, token: token}, function (res) {
      ...
      request('connector.miniHandler.minichinaenter', {gameId: 40003}, function (res) {
        ...
        request('connector.miniHandler.minichina', {gameId: 40003, index: 1}, function (res) {
          cc.log('获得中国风主题数据:');
          cc.log(JSON.stringify(res));
        });

        request('connector.miniHandler.minichina', {gameId: 40003, index: 2}, function (res) {
          cc.log('获得中国风主题数据:');
          cc.log(JSON.stringify(res));
        });

        request('connector.miniHandler.minichina', {gameId: 40003, index: 3}, function (res) {
          cc.log('获得中国风主题数据:');
          cc.log(JSON.stringify(res));
        });
      });
